    ...
    <div class="flex-1 grid grid-cols-3 gap-3 items-center">
      ...
      >
        <img
          :src="mode.icon"
          :class="[
            'w-10 h-10 object-contain transition-all duration-300 brightness-0 invert',
            mode.active ? 'opacity-100 scale-110' : 'opacity-40 group-hover:opacity-100',
          ]"
        />
        <span :class="['text-xs', mode.active ? 'text-white font-medium' : 'text-gray-500']">{{
          mode.name
        }}</span>
      </button>
    </div>
